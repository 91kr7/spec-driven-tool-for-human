...
- You open the **branch of the cycle**: this command is normally the first that writes, so the
  branch is yours to open → convention `${extensionPath}/conventions/branching.md`.

## Steps

1. Get the current date in ISO-8601 format with `date +%Y-%m-%d`.
2. Launch the `sdd-analyst` subagent via invoke_subagent, passing it the request (`$ARGUMENTS`) and the current
...
4. **Check the size** of the file produced → convention
   `${extensionPath}/conventions/analysis-shape.md`: read the `size:` class in its frontmatter
   and compare the budget with `wc -l`. Over budget → resume the same subagent with `send_message`,
...
5. **Open the branch** of the cycle, now that the slug and the type are known, following the
   convention `${extensionPath}/conventions/branching.md`. Before the commit, never after: the
